...
<?php
    ...
    if (isset($_POST['review'])) {
        ...
        // ข้อมูล
        $bg = $_POST['bg'];
        // $bg = "/app-certificate/backend/images/bg/bg-003.png";
        $name = "ชื่อ นามสกุล";
        $school = "";
        ...
        $pdf->setFontSize(28);
        $pdf->MultiCell(0, 0, 'สถาบันเทคโนโลยีพระจอมเกล้าเจ้าคุณทหารลาดกระบัง', 0, 'C', 0, 1, 0, 52);

        $pdf->setFont('thsarabun', '');
        $pdf->setTextColor(0, 98, 133);
        $pdf->setFontSize(20);
        $pdf->MultiCell(0, 0, 'ขอมอบประกาศนียบัตรฉบับนี้ไว้เพื่อแสดงว่า', 0, 'C', 0, 1, 0, 65);
        // ชื่อ
        $pdf->setFont('thsarabun', 'B');
        ...
        $pdf->setFontSize(24);
        $pdf->MultiCell(0, 0, $award, 0, 'C', 0, 1, 0, 105);
        
        // ชื่องาน
        $pdf->setFont('thsarabun', 'B');
        $pdf->setTextColor(40, 46, 75);
        $pdf->setFontSize(28);
        $pdf->MultiCell(0, 0, $event, 0, 'C', 0, 1, 0, 115);
        // วันที่จัดงาน
        $pdf->setFont('thsarabun', 'B');
        $pdf->setTextColor(40, 46, 75);
        $pdf->setFontSize(24);
        $pdf->MultiCell(0, 0, $event_date, 0, 'C', 0, 1, 0, 126);
    ...
    if (isset($_POST['submit'])) {
        if ($_FILES['excel']['error'] == 0) {
            ...
            function get($c)
            {
                $k = array("A" => "num", "B" => "student", "C" => "award", "D" => "event", "E" => "event_date");
                return $k[$c];
            }
